...
* **Trained weights exist for oil presence & look-alike review.** `ml_pipeline/segmentation/weights/varuna_oil_cnn_v1.pt` is a 294k-param GAP-CNN trained on the Sentinel-1 SAR Oil Spill Detection Dataset (5538 scenes; test acc 0.805, AUC 0.870, F1 0.733, oil recall 0.804 — see `training_metrics_v1.json`). At inference its CAM heatmap refines the classical contour when it agrees oil is present (`segmentation_source: "learned-cnn-cam-refined"`, else `"classical"` with capped confidence). An accompanying Stage-2 Look-Alike Modulator (`varuna_oil_lookalike_v2.pt`, trained on 807 mined hard negatives) flags marginal look-alikes, lifting confirmed oil precision to 69.3% while preserving 95.95% of true positives. The "ESRGAN" stage remains a classical 4x chain (no learned SR weights). Per-scene IoU/Dice against pixel masks is impossible (the dataset has image-level labels only).
