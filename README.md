...
* **Trained weights v1 exist for oil presence, not for pixel segmentation.** `ml_pipeline/segmentation/weights/varuna_oil_cnn_v1.pt` is a 294k-param GAP-CNN trained on the Sentinel-1 SAR Oil Spill Detection Dataset (5538 scenes, stratified 80/10/10, class-weighted; held-out test: acc 0.805, AUC 0.870, F1 0.733, oil precision 0.673 / recall 0.804 — see `training_metrics_v1.json`). At inference its CAM heatmap refines the classical contour when it agrees oil is present (`segmentation_source: "learned-cnn-cam-refined"`, else `"classical"` with capped confidence). The "ESRGAN" stage remains a classical 4x chain (no SR weights). Per-scene IoU/Dice against pixel masks is impossible (the dataset has image-level labels only), and dedicated look-alike (biogenic / low-wind / sediment) classification is still future work — the 0.67 oil precision shows why.
